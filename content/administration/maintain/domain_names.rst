...
However, you can use your own domain name instead, either by :ref:`registering a free domain name
...
.. important::
   ...
   provided in the :guilabel:`Domain Owner` form. Open the link contained in the email to verify
   your email address. The domain name will **stop** working after two weeks if the email address is
   left unverified.
...
Then, add the alias domain as an *own domain* by clicking :guilabel:`Use my own domain`, entering
...
field, enter the alias domain (e.g., `subdomain.yourdomain.com`) and click :guilabel:`Create`.
...
.. warning::
   It is strongly recommended to follow these three steps **in order** to avoid any :ref:`SSL
   certificate validation <domain-name/ssl>` issue:
...
.. example::
   ...
   target. Next, create a CNAME record to redirect visitors from `yourdomain.com` to
   `wwww.yourdomain.com`.
...
.. warning::
   Make sure you have :ref:`added a CNAME record <domain-name/cname>` to your domain name's DNS
   **before** mapping your domain name to your Odoo database.
   ...
   would result in a *certificate name mismatch* error. Web browsers often display this as a
...
.. important::
   ...
   or prefixes, e.g., `yourdomain.com`).
...
.. note::
   If you have Odoo Website installed, skip this section and continue from the :ref:`map a domain
   name to a website <domain-name/website-map>` section.
...
.. warning::
   ...
   base URL of your database will be updated accordingly. To prevent the automatic update of the web
   base URL when an administrator logs in the database, activate the :ref:`developer mode
...
.. note::
   ...
   `https://` (or `http://`) and must *not* end with a slash (`/`).
...
.. warning::
   Mapping your domain name to your Odoo website prevents Google Search from indexing both your
   custom domain name (e.g., `www.yourdomain.com`) and your original Odoo database address (e.g.,
   `mycompany.odoo.com`).
...
.. note::
   If you have multiple websites and companies on your database, make sure that you also select the
   right :guilabel:`Company` under :menuselection:`Website --> Configuration --> Settings`. Doing so
